...
const EMOJI_GROUPS = [
  ...
  { id: 2, label: "动物", icon: "🐾" },
  { id: 3, label: "食物", icon: "🍜" },
  { id: 4, label: "旅行", icon: "✈️" },
  { id: 5, label: "活动", icon: "⚽" },
  { id: 6, label: "物品", icon: "💡" },
  { id: 7, label: "符号", icon: "💬" },
  { id: 8, label: "旗帜", icon: "🏳️" },
] as const;

const EMOJI_RECENT_KEY = "astro-bento-blog.post-editor.emoji-recents";
const COMMANDS: Command[] = [
  { target: "paragraph", title: "文本", hint: "基础段落", aliases: ["text", "p", "文本", "段落"] },
  { target: "heading:1", title: "标题 1", hint: "大标题", aliases: ["h1", "标题1"] },
  { target: "heading:2", title: "标题 2", hint: "次级标题", aliases: ["h2", "标题2"] },
  { target: "heading:3", title: "标题 3", hint: "小标题", aliases: ["h3", "标题3"] },
  { target: "todo", title: "待办", hint: "复选框", aliases: ["todo", "check", "待办", "勾选"] },
  { target: "toggle", title: "折叠", hint: "可折叠列表", aliases: ["toggle", "折叠", "折叠"] },
  { target: "quote", title: "引用", hint: "引用文字", aliases: ["quote", "引用"] },
  { target: "callout", title: "提示框", hint: "强调提示", aliases: ["callout", "note", "提示框", "备注"] },
  { target: "code", title: "代码", hint: "代码块", aliases: ["code", "代码"] },
  { target: "divider", title: "分隔线", hint: "水平线", aliases: ["divider", "hr", "分隔线"] },
  { target: "table_of_contents", title: "目录", hint: "在此位置显示标题目录", aliases: ["toc", "contents", "目录"] },
  { target: "image", title: "图片", hint: "图片 URL", aliases: ["image", "img", "图片"] },
  { target: "bookmark", title: "书签", hint: "链接预览", aliases: ["bookmark", "link", "书签", "链接"] },
  { target: "equation", title: "公式", hint: "LaTeX 公式块", aliases: ["math", "equation", "latex", "公式"] },
  { target: "table", title: "表格", hint: "3 x 3 编辑表格", aliases: ["table", "表格", "表格"] },
  { target: "context", title: "背景说明", hint: "整理背景与前提", aliases: ["context", "背景说明", "上下文"] },
  { target: "emoji-menu", title: "表情", hint: "搜索并插入表情", aliases: ["emoji", "icon", "表情", "图标"] },
];
...
export function initPostEditor(): void {
  ...
  function sampleDocument(): EditorDocument {
    return {
      ...
      blocks: [
        createEditorBlock("heading", "问题定义", { level: 1 }),
        createEditorBlock("paragraph", "先写背景，一个块只表达一个想法。"),
        createEditorBlock("callout", "此编辑器将富文本与块属性保存为 JSON v2。", {
          backgroundColor: "teal",
        }),
        createEditorBlock("heading", "实现笔记", { level: 2 }),
        createEditorBlock("bulleted_list", "输入 / 即可搜索块命令菜单。"),
        createEditorBlock("todo", "保存为 JSON 并传递给发布流程"),
  ...
  function renderStatusPicker(): void {
    ...
    statusTrigger.textContent = statuses.length ? `已选择 ${statuses.length} 个状态` : "选择状态";
    statusChips.replaceChildren(...statuses.map((status) => {
      ...
      chip.setAttribute("aria-label", `移除 ${status}`);
  ...
  function renderTable(block: EditorBlock): string {
    ...
    return `<div class="editor-table-wrap">
      ...
      <div class="editor-table-actions">
        <button type="button" data-table-action="add-row">添加行</button>
        <button type="button" data-table-action="add-col">添加列</button>
        <button type="button" data-table-action="remove-row">删除行</button>
        <button type="button" data-table-action="remove-col">删除列</button>
      </div>
    </div>`;
  }
  ...
  function renderBlock(block: EditorBlock, depth: number, childrenMarkup = ""): string {
        ...
        <div class="editor-image-surface">
          <div class="editor-image-stage" data-image-stage>
            <div class="editor-image-preview${imageSrc ? "" : " is-empty"}" data-image-preview${widthStyle}>
              ...
              <output class="editor-image-size-label" data-image-size-label>${displayWidth ? `${displayWidth}px` : "正文宽度"}</output>
              <button type="button" class="editor-image-resize-handle editor-image-resize-handle--left" data-image-resize="left" aria-label="调整图片大小"></button>
              <button type="button" class="editor-image-resize-handle editor-image-resize-handle--right" data-image-resize="right" aria-label="调整图片大小"></button>
            </div>
          </div>
          <div class="editor-image-fields">
            <label><span>图片 URL</span><input data-field="src" value="${escapeHtml(block.src ?? "")}" placeholder="/image.webp" /></label>
            <label><span>替代文本</span><textarea data-field="alt" rows="2" placeholder="图片说明">${escapeHtml(block.alt ?? "")}</textarea></label>
            <label><span>图注</span><textarea data-field="caption" rows="2" placeholder="可选">${escapeHtml(getRichTextPlainText(block.caption))}</textarea></label>
            <div class="drive-image-upload" data-drive-image-actions>
              ...
              <div class="editor-image-size-control">
                <label><span>显示宽度</span><input type="number" inputmode="numeric" data-image-width min="${IMAGE_MIN_DISPLAY_WIDTH}" max="${IMAGE_MAX_DISPLAY_WIDTH}" step="10" value="${displayWidth ?? ""}" placeholder="正文宽度" /></label>
                <button type="button" data-image-width-reset>恢复为正文宽度</button>
              </div>
            </div>
          </div>
        </div>
    ...
    if (block.type === "bookmark") {
      return `<section class="editor-block editor-block--bookmark${shellClass}" data-id="${block.id}" data-depth="${depth}">
        ...
        <label><span>标题</span><input data-field="title" value="${escapeHtml(block.title ?? "Bookmark")}" /></label>
        <label><span>描述</span><input data-field="description" value="${escapeHtml(block.description ?? "")}" /></label>
      </section>`;
    }
    ...
    if (block.type === "equation") {
      ...
      return `<section class="editor-block editor-block--equation${shellClass}" data-id="${block.id}" data-depth="${depth}">
        ...
        <div class="editor-equation-preview" data-equation-preview aria-label="公式预览">${renderEquationHtml(equation)}</div>
      </section>`;
    }
    ...
    if (block.type === "toggle") {
      ...
      return `<section class="editor-block editor-block--toggle${shellClass}" data-id="${block.id}" data-depth="${depth}">${renderControls()}<section class="editor-toggle"><div class="editor-toggle__header"><button type="button" class="editor-toggle__disclosure" data-toggle-action="toggle" aria-label="折叠内容 ${isOpen ? "折叠" : "展开"}" aria-expanded="${isOpen}"></button>${richRootMarkup(block, "折叠标题", "span")}</div><div class="editor-block-children editor-toggle__children"${isOpen ? "" : " hidden"}>${childrenMarkup || `<button type="button" class="editor-toggle__empty" data-toggle-empty>请输入内容</button>`}</div></section></section>`;
    }
    ...
    if (block.type === "table_of_contents") {
      return `<section class="editor-block editor-block--table-of-contents${shellClass}" data-id="${block.id}" data-depth="${depth}">
        ...
        <div class="editor-toc-preview">
          <strong>目录</strong>
          <span>发布时在此位置以 Bento 卡片显示文档标题。</span>
        </div>
      </section>`;
    }
    ...
    if (block.type === "context") {
      return `<section class="editor-block editor-block--context${shellClass}" data-id="${block.id}" data-depth="${depth}">
        ${renderControls()}
        <input class="editor-context-title" data-field="title" value="${escapeHtml(block.title ?? "背景说明")}" aria-label="背景说明标题" />
        ${richRootMarkup(block, "写下背景、前提和参考说明。")}
      </section>`;
    }
    ...
    if (block.type === "code") {
      return `<section class="editor-block editor-block--code${shellClass}" data-id="${block.id}" data-depth="${depth}">
        ${renderControls()}
        <input class="editor-code-lang" data-field="language" value="${escapeHtml(block.language ?? "text")}" aria-label="代码语言" />
    ...
    const checkbox = block.type === "todo"
      ? `<input type="checkbox" data-field="checked" ${block.checked ? "checked" : ""} aria-label="完成" />`
      : "";
  ...
  function normalizeStoredBlock(rawValue: unknown): EditorBlock | null {
    ...
    if (type === "context") options.title = String(raw.title ?? "背景说明");
  ...
  function renderEmojiPicker(query = ""): void {
    ...
    const emptyLabel = query ? "没有搜索结果。" : "没有最近使用的表情。";
    emojiMenu.innerHTML = `<section class="emoji-picker" aria-label="表情选择器">
      <div class="emoji-picker__search-row">
        <input type="search" data-emoji-search value="${escapeHtml(query)}" placeholder="搜索表情" aria-label="搜索表情" autocomplete="off" />
      </div>
      <div class="emoji-picker__tabs" role="tablist" aria-label="表情分类">
        <button type="button" class="emoji-picker__tab${emojiGroup === -1 && !query ? " is-active" : ""}" data-emoji-group="-1" title="最近使用">◷</button>
  ...
  async function ensureEmojiPicker(): Promise<void> {
    if (!emojiData) {
      emojiMenu.innerHTML = `<div class="emoji-popover__loading">正在加载表情数据。</div>`;
  ...
  async function openEmojiMenu(mode: "page" | "block" | "inline", blockId: string, anchor: HTMLElement, insertionOffset?: number): Promise<void> {
    ...
    try {
      ...
      emojiMenu.innerHTML = `<div class="emoji-popover__error">无法加载表情数据。<br />${escapeHtml(message)}</div>`;
      positionFloating(emojiMenu, anchor.getBoundingClientRect(), "below");
    }
  }
  ...
  function coverMenuMarkup(): string {
    ...
    return `<div class="cover-menu__title">封面</div>
      <div class="cover-menu__section">
        <span class="cover-menu__label">颜色</span>
      ...
      <form class="cover-menu__section" data-cover-url-form>
        <label class="cover-menu__label" for="cover-url">图片链接</label>
      ...
      ${cover ? `<button type="button" class="cover-menu__remove" data-cover-remove>移除封面</button>` : ""}`;
  }
  ...
  function renderColorMenu(mode: "block" | "text", activeValue = "default"): string {
    const title = mode === "text" ? "文字颜色" : "块背景色";
  ...
  function renderSlashMenu(anchor?: HTMLElement): void {
    slashMenu.innerHTML = slashState.items.length
      ? slashState.items.map((command, index) => `
        ...
        `).join("")
      : `<div class="slash-menu__empty">没有匹配的命令。</div>`;
  ...
  function showBlockActionMenu(blockId: string, anchor: HTMLElement): void {
    ...
    blockContextMenu.innerHTML = `
      <button type="button" data-context-action="color">设置颜色</button>
      <button type="button" data-context-action="copy-url">复制指向此块的链接</button>
      <button type="button" class="block-action-menu__delete" data-context-action="delete">删除块</button>
    `;
  ...
  function updateImageWidthUI(element: HTMLElement, displayWidth?: number): void {
    ...
    if (label) label.textContent = "正文宽度";
  ...
  function serializedBlock(block: EditorBlock): Record<string, unknown> {
    ...
    if (block.type === "context") base.title = block.title ?? "背景说明";
    if (block.type === "code") {
  ...
  async function copyBlockUrl(blockId: string): Promise<void> {
    try {
      await copyText(getBlockUrl(blockId), "块链接");
    } catch {
      showToast("无法复制链接。");
    }
  }
  ...
  async function importJsonFile(file: File): Promise<void> {
    try {
      ...
      render();
      scheduleSave();
      showToast(`${file.name} 已导入`);
    } catch {
      showToast("只能导入 Post JSON v2 文件。");
    } finally {
